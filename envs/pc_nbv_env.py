...
class PointCloudNextBestViewEnv(gym.Env):
    def __init__(self,
        ...
        self.part_tensor = tf.placeholder(tf.float32, (1, None, 3))
        self.gt_tensor = tf.placeholder(tf.float32, (1, None, 3))
        self.config = tf.ConfigProto()
        self.config.gpu_options.allow_growth=True
        self.sess = tf.Session(config=self.config)
        self.dist1, _, self.dist2, _ = tf_nndistance.nn_distance(self.part_tensor, self.gt_tensor)
        ...
        self.gt = resample_pcd(self.ground_truth_points_cloud, 1024, self.logger, "1024")
    ...
    def step(self, action):
        ...
        batch_acc = new_points_cloud_tensor
        ...
        batch_gt = cur_points_cloud_tensor
        # cur_points_cloud_tensor = torch.tensor(cur_points_cloud_tensor).to(self.DEVICE)
        dist1 = self.sess.run(self.dist1, feed_dict={self.part_tensor:batch_acc, self.gt_tensor:batch_gt})      
        ...
        batch_acc = increase_points_tensor
        batch_gt = self.ground_truth_points_cloud[np.newaxis, :]
        dist2 = self.sess.run(self.dist2, feed_dict={self.part_tensor:batch_acc, self.gt_tensor:batch_gt})      
    ...
    def _caculate_current_coverage(self):
        ...
        # dist2 = dist2.cpu().numpy()   
        batch_acc = cur_points_cloud_tensor
        batch_gt = self.ground_truth_points_cloud[np.newaxis, :]
        dist2 = self.sess.run(self.dist2, feed_dict={self.part_tensor:batch_acc, self.gt_tensor:batch_gt})     
        cover_flag = dist2 < self.COVERAGE_THRESHOLD
        # cover_flag = cover_flag[0, :]
    ...
    def _get_observation_space(self):
        if self.observation_space_dim == -1:
            ...
            cur_pc = self.current_points_cloud_from_gt
            ...
            cur_pc = cur_pc[np.newaxis, :]
            cur_view_state = self.view_state
            cur_view_state = cur_view_state[np.newaxis, :]
            # for PC_NBV net
            # cur_pc = cur_pc.T
            return {"current_point_cloud": cur_pc, "view_state": cur_view_state}
